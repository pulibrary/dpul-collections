defmodule DpulCollectionsWeb.SearchLive do
  ...
  def render(assigns) do
    ...
    <div id="search-results" class="grid grid-flow-row auto-rows-max gap-8">
      <div :if={@loading} id="loader" class="w-100% text-center" role="status">
          <svg aria-hidden="true" class="w-16 h-16 inline text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  def search_item(assigns) do
    ~H"""
    <hr />
    <div id={@item.id} class="item" phx-hook="Fade" >
